...
from src.core.packages.packages import is_installed, install_package, remove_package 
from src.core.security.sudo_keep_alive import SudoKeepAlive
...
class PackagesInstaller:
    ...
    def install(self) -> bool:
        self.logger.info("Packages installer started")
        print_header(
            "Installing packages.",
            details="Installing packages(dependencies), This might take a while depending on your internet speed. please be patient.",
        )
        print_newline()
        ...
        if conflicts and not self.dry_run:
            ...
            for pkg in conflicts:
                with Spinner(f"Removing {pkg}", verbose=self.verbose) as spinner:
            ...
            if failed_removals:
                print_error(
                    ...
                    details="Installation will continue but SBDots may not work properly.\nPlease remove them manually!",
                )
            else:
        ...
        for title, packages_list in groups.items():
            with Spinner(f"Installing {title}", verbose=self.verbose) as spinner:
                sleep(1)
    ...
    def install_optional_applications(self) -> bool:
        ...
        sleep(1)
        chosen: list[str] | str | None = check_box(
            choices=self.optional, message="Choose apps to install."
        )
